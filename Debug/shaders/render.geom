...
void main()
{
    ...
    texCoord = vec2(1.0 * texColumn / texColumnCount, 1.0 * texRow / texRowCount);
    gl_Position = mVP * vec4(posLeftBottom, 1.0);
    EmitVertex();

    vec3 posLeftTop = posCenter + (-quad1 + quad2) * size[0];
    texCoord = vec2(1.0 * texColumn / texColumnCount, 1.0 * (texRow + 1) / texRowCount);
    gl_Position = mVP * vec4(posLeftTop, 1.0);
    EmitVertex();

    vec3 posRightBottom = posCenter + (quad1 - quad2) * size[0];
    texCoord = vec2(1.0 * (texColumn + 1) / texColumnCount, 1.0 * texRow / texRowCount);
    gl_Position = mVP * vec4(posRightBottom, 1.0);
    EmitVertex();

    vec3 posRightTop = posCenter + (quad1 + quad2) * size[0];
    texCoord = vec2(1.0 * (texColumn + 1) / texColumnCount, 1.0 * (texRow + 1) / texRowCount);
